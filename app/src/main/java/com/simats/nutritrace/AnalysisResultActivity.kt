...
class AnalysisResultActivity : AppCompatActivity() {
        ...
        if (imageUriString != null) {
            ...
            if (scanJsonStr != null) {
                ...
                if (!imagePath.isNullOrEmpty()) {
                    ...
                    binding.cvThumbnail.setOnClickListener { showImagePopup(imagePath) }
                }
            }
        }
    ...
    private fun populateFromApi() {
        ...
        // Stat cards
        var sugarEstimate = scanObj.get("sugar_estimate")?.asString ?: "--"
        if (sugarEstimate.contains(" ") || sugarEstimate.length > 10) {
            sugarEstimate = sugarEstimate.split(" ", "(", ",", limit = 2).firstOrNull()?.replace(Regex("[^a-zA-Z0-9]"), "") ?: "--"
        }
    ...
    private fun showImagePopup(uriOrPath: String?) {
        ...
        // Local URIs from the photo picker or camera start with content:// or file://
        if (uriOrPath.startsWith("content://") || uriOrPath.startsWith("file://") || uriOrPath.startsWith("content:")) {
            try {
                ivFullImage.setImageURI(android.net.Uri.parse(uriOrPath))
            } catch (e: Exception) {
                e.printStackTrace()
            }
        } else {
            // Otherwise, it's a server image path (either full HTTP url or relative uploads/ path)
            ApiClient.loadImage(ivFullImage, uriOrPath)
        }
