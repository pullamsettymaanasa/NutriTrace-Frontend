...
class NewPasswordActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnResetPassword.setOnClickListener {
            ...
            if (!isPasswordValid()) {
                android.widget.Toast.makeText(this, "Please meet all password requirements", android.widget.Toast.LENGTH_SHORT).show()
    ...
    private fun setupValidations() {
        val textWatcher = object : android.text.TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                // validation checks are on click only now
            }
    ...
    private fun isPasswordValid(): Boolean {
        ...
        return p.length >= 8 && p.any { it.isUpperCase() } && p.any { it.isLowerCase() } && p.any { it.isDigit() } && p.any { !it.isLetterOrDigit() }
    }
