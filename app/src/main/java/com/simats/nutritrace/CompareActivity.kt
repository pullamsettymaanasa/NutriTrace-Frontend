...
class CompareActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        androidx.core.view.ViewCompat.setOnApplyWindowInsetsListener(binding.root) { view, windowInsets ->
            ...
            val originalBottomPadding = (12 * density).toInt()
            
            navContainer.setPadding(navContainer.paddingLeft, navContainer.paddingTop, navContainer.paddingRight, originalBottomPadding + insets.bottom)
            bottomNavCard.post {
    ...
    private fun showImageDialog(imageUriString: String?) {
        ...
        if (imageUriString.startsWith("content://") || imageUriString.startsWith("file://") || imageUriString.startsWith("content:")) {
            try {
                ivFullImage.setImageURI(android.net.Uri.parse(imageUriString))
            } catch (e: Exception) {
                e.printStackTrace()
            }
        } else {
            ApiClient.loadImage(ivFullImage, imageUriString)
        }
