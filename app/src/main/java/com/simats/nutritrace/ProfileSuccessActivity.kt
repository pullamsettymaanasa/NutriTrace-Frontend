...
class ProfileSuccessActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.tvAgeGroup.text = ageGroup
        
        val displayMap = mapOf(
            "blood_sugar" to "Blood Sugar Regulation",
            "cardiovascular" to "Cardiovascular & Circulatory Health",
            "hormonal" to "Hormonal & Metabolic Health",
            "digestive" to "Digestive & Organ Health",
            "allergy_immune" to "Allergy & Immune Sensitivity",
            "no_specific" to "No Specific Conditions",
            "not_sure" to "Not Sure"
        )
        val formattedConditions = conditions.map { displayMap[it] ?: it }
        if (formattedConditions.isNotEmpty()) {
            binding.tvConditions.text = formattedConditions.joinToString(", ")
        } else {
            binding.tvConditions.text = "None"
        }
        
        val allSensitivities = ArrayList(sensitivities)
        allSensitivities.addAll(customSensitivities)
        if (allSensitivities.isNotEmpty()) {
            binding.tvSensitivities.text = allSensitivities.joinToString(", ")
        } else {
            binding.tvSensitivities.text = "None"
        }
